...
class ChurchControlSupabaseUser extends BaseAuthUser {
  ChurchControlSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> churchControlSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ChurchControlSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
